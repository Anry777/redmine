...
class AgileSprintQueriesController < ApplicationController
  helper :queries
  ...
  def find_query
    @query = AgileSprintsQuery.find(params[:id])
  ...
  def redirect_to_agile_versions_board(options)
    redirect_to sprints_project_agile_versions_path(options.merge(project_id: @project))
  end
end
